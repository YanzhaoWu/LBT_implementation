# -*- coding: utf-8 -*-
"""
Created on Thu Dec 22 17:39:44 2016
...
hdf5_data_path = 'train_data_full_.hdf5'
...
net = caffe.Net('LBT_deploy.prototxt', 'lbt__iter_5000.caffemodel', caffe.TEST)
train_samples = np.zeros((labels.shape[0], 516))
for i in range(0, y.shape[0]):
    
    x0 = np.array([locations[i][0][0], locations[i][0][2]])
    x1 = np.array([locations[i][1][0], locations[i][1][2]])
    s0 = np.array([locations[i][0][2] - locations[i][0][0], locations[i][0][3] - locations[i][0][1]])
    s1 = np.array([locations[i][1][2] - locations[i][1][0], locations[i][1][3] - locations[i][1][1]])
    t0 = times[i][0]
    t1 = times[i][1]
...
import h5py
def save_data_as_hdf5(hdf5_data_filename, data, label, location, time_stamp, cnn_prediction):
    '''
...
save_data_as_hdf5('train_data_full_with_prediction.hdf5', train_data_img, labels, locations, times, train_samples[:, 4:516])
